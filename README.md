...
First you will to generate a registration token. To do so, launch the server and use the following command and keep the registrationToken for late:

```shell
curl -X POST http://127.0.0.1:8000/api/v1/runners/registration-tokens/generate
```
...
Launch the runner
```shell
./packages/peertube-runner/dist/peertube-runner.js server
```

Open a new terminal in the same directory and register your runner to your django app
```shell
./packages/peertube-runner/dist/peertube-runner.js register --url http://localhost:8000 --registration-token $MY_TOKEN --runner-name transcode-api
```
